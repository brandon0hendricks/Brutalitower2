...
public class RandomUpgrade : MonoBehaviour
{
    ...
    //[SerializeField]
    //private List<Sprite> upgradeIcons; //what is the icon

    [SerializeField]
    private TMPro.TMP_FontAsset upgradeFont; //font for the upgrade text

    [SerializeField]
    private Sprite healthIcon; //icon for health upgrade
    [SerializeField]
    private Sprite baseHealthIcon;
    [SerializeField]
    private Sprite damageIcon;
    [SerializeField]
    private Sprite baseDamageIcon;
    [SerializeField]
    private Sprite dodgeIcon; //icon for dodge upgrade
    [SerializeField]
    private Sprite baseDodgeIcon; //icon for base dodge upgrade
    ...
    private int upgradeFontSize = 4; // Font size for the upgrade text
    private int currencyFontSize = 1; // Font size for the currency text

    private void randomizeUpgrade()
    {
        isPurchasable = true; // Default to purchasable, can be set to false if needed
        float baseChance = Random.Range(0f, 1f);
        if (baseChance < 0.25f)
        {
            isBaseUpgrade = true; // 25% chance to be a base upgrade
            purchaseType = "bonedust"; // Base upgrades are always purchased with bonedust
            upgradePrice = 10;
        }
        else
        {
            isBaseUpgrade = false; // 75% chance to be a regular upgrade
            purchaseType = "goldcoin"; // Regular upgrades are purchased with gold coins
            upgradePrice = 25;
        }
        float typeChance = Random.Range(0f, 1f);
        if (typeChance < 0.33f)
        {
            upgradeType = "hp"; // 25% chance to be a health upgrade
            upgradeAmount = 10; // Default amount for health upgrades
        }
        else if (typeChance < 0.67f)
        {
            upgradeType = "damage"; // 25% chance to be a damage upgrade
            upgradeAmount = 1; // Default amount for damage upgrades
        }
        else
        {
            upgradeType = "dodge"; // 25% chance to be a dodge upgrade
            upgradeAmount = 1; // Default amount for dodge upgrades
        }
    }
    ...
    private void setIcon()
    {
        switch (upgradeType)
        {
            case "hp":
                if (isBaseUpgrade)
                {
                    upgradeIcon = baseHealthIcon; // Base health upgrade icon
                }
                else
                {
                    upgradeIcon = healthIcon; // Regular health upgrade icon
                }
                break;
            case "damage":
                if (isBaseUpgrade)
                {
                    upgradeIcon = baseDamageIcon; // Base damage upgrade icon
                }
                else
                {
                    upgradeIcon = damageIcon; // Regular damage upgrade icon
                }
                break;
            case "dodge":
                if (isBaseUpgrade)
                {
                    upgradeIcon = baseDodgeIcon; // Base dodge upgrade icon
                }
                else
                {
                    upgradeIcon = dodgeIcon; // Regular dodge upgrade icon
                }
                break;
        }
    }
    ...
    public void Start()
    {
        randomizeUpgrade(); // Randomly select the upgrade type, amount, and price
        makeDescriptions(); // Initialize upgrade descriptions
        setIcon(); // Set the icon based on the upgrade type and whether it's a base upgrade
    ...
    private void configureUpgradeUI()
    {
        ...
        // Set the font for the text components
        upgradeTextComponent.font = upgradeFont; // Set the font for upgrade text
        currencyTextComponent.font = upgradeFont; // Set the font for currency text
        ...
        //Center the text for upgrade descriptions
        upgradeTextComponent.alignment = TMPro.TextAlignmentOptions.Center; // Center alignment for upgrade text
    }
    /*
